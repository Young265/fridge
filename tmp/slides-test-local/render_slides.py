...
def _render_presentation_with_artifact_tool(
    ...
    # On this managed Windows runtime artifact-tool can render every slide and
    # emit a valid JSON payload, then still exit non-zero during workspace
    # cleanup. Accept the completed render when every reported file exists;
    # otherwise preserve the original hard failure.
    try:
    ...
    if proc.returncode != 0:
        ...
        if not paths or not all(exists(path) for path in paths):
    ...
    if payload is None:
        raise RuntimeError("Artifact-tool returned no JSON render payload.")
